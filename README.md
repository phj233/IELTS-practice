...
通过该应用地址打开页面后，页首会出现“登录并同步”入口。首次使用时注册账号；登录后，浏览器已有的练习记录会与服务器记录按记录 ID 和更新时间合并，并写回本地存储。之后新增和删除练习记录会在后台同步；网络不可用时仍保留本地练习和本地存储作为回退。请始终从同一个应用地址访问页面，不要把静态前端和 API 分别部署到不同域名。
