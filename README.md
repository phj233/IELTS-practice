...
`main` 同时提供两种互不冲突的使用方式；不部署后端时，它仍是可直接打开或静态托管的单机练习系统。
...
多设备同步功能已包含在 `main` 中，不要求切换到单独的功能分支。未部署或暂时无法访问同步服务时，单机练习、题库使用和本地记录不会受影响。

| 分支 | 说明 | 状态 | 完成度 | 技术特征 |
|------|------|------|--------|----------|
| [main](https://github.com/sallowayma-git/IELTS-practice/tree/main) | 当前稳定分支；默认可静态单机运行，并内置可选的多设备同步部署能力 | ![状态](https://img.shields.io/badge/状态-稳定-success) | ![完成度](https://img.shields.io/badge/完成度-95%25-brightgreen) | ![技术](https://img.shields.io/badge/技术-静态前端__可选_Node.js-blue) |
...
> **main** 适合所有用户：直接使用即为单机模式；只有需要跨设备同步练习记录时，才需要继续部署后端。**IELTS-WRITING-FEAT** 是独立的 AI native 协作客户端分支。
...
这是可选部署，不是运行本项目的前置条件。不开启同步时，无需配置环境变量、容器、账号或数据库；继续按上面的“本地直接运行”“本地静态服务器运行”或“静态网页部署”使用即可。`file://` 页面始终按单机模式运行，不显示网络同步入口。
...
```bash
git clone https://github.com/phj233/IELTS-practice.git /opt/ielts-practice
